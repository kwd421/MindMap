...
class ReviewReader:
    ...
    def answer(self, *, question: str, context: str) -> ExtractiveReaderResult:
        ...
        answer = (
            "migraine"
            if "migraine" in context.casefold()
            else "stitches"
            if "stitches" in context.casefold()
            else ""
        )
...
def test_deictic_unscoped_deny_is_unknown_admit_but_directive_is_not_evidence() -> None:
...
def test_primary_contract_is_retrospective_not_prospective() -> None:
