...
def _note(base: str, head: str, branch: str) -> str:
...
def test_valid_authorship_note_binds_base_head_and_two_paths(tmp_path: Path):
    base = "1" * 40
    head = "2" * 40
    path = tmp_path / "AUTHORSHIP.md"
    path.write_text(
        _note(base, head, "research/track-x-v0.2-heldout-session-a"),
        encoding="utf-8",
    )
    ...
    assert declaration.heldout_commit == head
...
def test_authorship_note_rejects_wrong_base_or_unreserved_branch(tmp_path: Path):
    path = tmp_path / "AUTHORSHIP.md"
    path.write_text(
        _note("1" * 40, "2" * 40, "research/unreserved"),
        encoding="utf-8",
    )
