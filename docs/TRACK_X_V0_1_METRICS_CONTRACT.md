...
```text
C_5,1 = maximum coverage
        subject to ordinary risk <= 0.05
                   governance risk <= 0.01
```

The threshold is selected on development topologies and applied unchanged to the held-out test. The all-abstain policy is always available and has zero coverage.
...
The scaffold supplies Brier score and fixed-width expected calibration error. Confidence represents the probability that an attempted answer is correct and permissible. An abstention is not counted as a correct answer.

Confirmatory reporting should also include a full risk-coverage curve and confidence-bin counts. ECE alone is not a sufficient calibration claim.
...
5. The scaffold does not yet render dialogue, call a model, update memory, retrieve evidence, or judge answers.
6. No result should be reported from the unit-test examples.
...
- freeze development-selected alignment weights, threshold, and confidence policy;
