...
**Tracks:** Issue #4  
**Base:** stable endpoint/selective-audit freeze `e6bef675c43e77ae0a922b8461aa3ac403a32dfd`
...
## 6. Base correction record

The first B1b branch was stacked on PR #46 head
`ae1fc83bade5709e3671c47b9cb251074650b066`. CI showed that its two post-result
query-ID-removal commits had not updated the RPC decoder and pre-existing tests:
ordinary CI failed before evaluating B1b.

No B1b result from that attempt is accepted. This branch is rebuilt on
`e6bef675c43e77ae0a922b8461aa3ac403a32dfd`, the immediately preceding endpoint
and answered-denominator freeze. It retains the reproduced B0/B1a scorer and
selective-audit surface while avoiding the unrelated, incomplete capability
migration. A future query-ID removal must pass its own compatibility review.
